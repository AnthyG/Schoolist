...
function EPs(req, res) {
    var fpathG = dir + '/DB/Eventgroups/' + req.query.eventg + '/group.json';
    var fpath = false;
    if (typeof req.query.event !== 'undefined' && typeof req.query.event !== 'null') {
        fpath = dir + '/DB/Eventgroups/' + req.query.eventg + '/' + req.query.event + '.json';
    }
    ...
    if (fs.existsSync(fpathG)) {
        ...
        if (jsonFG.classes.indexOf(req.session.user_class) !== -1) {
            if (fpath === false) {
                ...
                var eventslist_HTML = '';
                var eventslist_arr = fs.readdirSync(dir + '/DB/Eventgroups/' + req.query.eventg);
                eventslist_arr.forEach(function(E) {
                    if (E !== 'group.json') {
                        var E = E.slice(0, -5);
                        eventslist_HTML += '<li><a href="?eventg=' + req.query.eventg + '&event=' + E + '">' + E + '</a></li>';
                    }
                });
...
function rPP(req, res) {
    var eg = '';
    if (typeof req.query.eventg !== 'undefined' && typeof req.query.eventg !== 'null') {
        eg += 'eventg=' + req.query.eventg;
    }
    if (typeof req.query.event !== 'undefined' && typeof req.query.event !== 'null') {
        eg += '&event=' + req.query.event;
    }
...
app.get('/panel', function(req, res) {
    if (req.query.logout) {
        doLogout(req, res);
    } else if (isLI(req) && typeof req.query.eventg !== 'undefined' && typeof req.query.eventg !== 'null') {
        EPs(req, res);
    } else if (isLI(req)) {
        PP(req, res);
    } else {
...
//     rPP(req, res);
// });
app.post(/^(?:(?!\.js|\.css).)*$/, function(req, res, next) {
    console.log("POST /");
    if (isLI(req)) {
        rPP(req, res);
    } else {
